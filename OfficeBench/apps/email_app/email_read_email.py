...
def read_email(username, email_id, workdir=None):
    """
    Read a user's email by the given Email ID.
    """
    ...
    try:
        if not email_id.endswith('.eml'):
            email_id += '.eml'
        email_file = f'{testbed_dir}/emails/{username}/{email_id}'
        ...
        print('!!!', e)
        return 'Error: [email] Failed to read email.'


def main(username, email_id, workdir=None):
    message = read_email(username, email_id, workdir=workdir)
    if message != 'Error: [email] Failed to read email.':
