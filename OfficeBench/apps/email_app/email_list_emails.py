...
def list_emails(username, first_n_letters=20, workdir=None):
    """
    List emails for a given username.
    """
    ...
    try:
        email_folder = f'{testbed_dir}/emails/{username}'
        ...
        return 'Error: [email] Failed to list emails.'



def main(username, workdir=None):
    message = list_emails(username, workdir=workdir)
    if message == 'No emails found.':
        observation = f"OBSERVATION: No emails found for {username}."
    elif message != 'Error: [email] Failed to list emails.':
