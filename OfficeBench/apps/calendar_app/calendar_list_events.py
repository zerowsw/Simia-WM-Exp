...
def list_events(username, workdir=None):
    ...
    try:
        ...
        return 'Error: [calendar] Failed to list events.'

def main(username, workdir=None):
    if username == 'Multiple users':
        observation = f"OBSERVATION: Error: Failed to list events for {username}. Only support one user."
        return observation
    message = list_events(username, workdir=workdir)
    if message == 'Error: [calendar] Failed to list events.':
        observation = f"OBSERVATION: Error: [calendar] Failed to list events for {username}."
    elif message == '':
