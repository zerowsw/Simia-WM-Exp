...
# the longest-running process is pdf_convert_to_word which takes ~58 seconds to process a 13 page document.
...
class OfficeAgentEnv(IntercodeEnv):
    ...
    def __init__(self, image_name: str, container_name: str, exp_config: BlueprintConfig, **kwargs):
    ...
    def _prepare_docker_apps(self, app_dir):
        # copy token to app dir
        copy_token_cmd = f"scp .msal_tokens.json {app_dir}/.msal_tokens.json"
        copy_api_cmd = f"scp utils/substrate_api.py {app_dir}/substrate_api.py"

        # we'll dump this config file and pass the name so that we don't need to dump multiple times
        print ('preparing temp config file')
        with tempfile.NamedTemporaryFile(delete=False, suffix=".yaml", mode='w') as temp_file:
            save_blueprint_config(self.exp_config, temp_file)
            self.temp_file_path = temp_file.name
        copy_exp_config_cmd = f"scp {self.temp_file_path} {app_dir}/exp_config.yaml"
        
        subprocess.run(copy_token_cmd, shell=True)
        subprocess.run(copy_api_cmd, shell=True)
        subprocess.run(copy_exp_config_cmd, shell=True)
        command = [
        ...
        # BUG: There is a race condition since other containers may be copying exp_config.yaml
        # WE should run the commands to populate apps/ from main and then always know it won't change at copy time.
        print ('removing exp config file...')
        del_temp_file_cmd = f"rm {self.temp_file_path}"
        del_exp_config_cmd = f"rm {app_dir}/exp_config.yaml"
        # subprocess.run(del_temp_file_cmd, shell=True)
        #subprocess.run(del_exp_config_cmd, shell=True)
        #print ('exp config file removed...')
    ...
    def cache_docker_status(self, local_cache_dir, remote_cache_dir="/testbed"):
        ...
        result = subprocess.run(command, capture_output=True, text=True)
        assert result.returncode == 0, f"Cache Status: Failed to copy cache from container: {result.stderr} [Command: {command}]"
    ...
    def exec_action(self, action_string: str) -> None:
        ...
        try:
            if self.exp_config.experiment.use_thinking_tokens:
                ...
                match = re.match(r"<think>(.*?)</think>\s*<action>(.*)</action>", action_string)
                if match:
            ...
            if self.exp_config.experiment.use_thinking_tokens:
                self.history.append(({"thought": thinking_string, "action": action}, self.observation))
            else:
            ...
            with open(f'{self.exp_config.experiment.exp_dir}/error.log', 'a') as f:
